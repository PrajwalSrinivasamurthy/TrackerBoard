...
export const storage = {
  async get(key) {
    const res = await fetch(`${import.meta.env.BASE_URL}api/kv/${encodeURIComponent(key)}`);
  ...
  async delete(key) {
    const res = await fetch(`${import.meta.env.BASE_URL}api/kv/${encodeURIComponent(key)}`, { method: "DELETE" });
